...
def process_pulse_addition():
    """Menangani akumulasi pulsa dan koreksi pulsa setelah sekian detik."""
    global total_pulses

    if total_pulses == 0:
        return None  # Tidak ada pulsa baru, tidak perlu proses

    time.sleep(1)  # 🕒 Delay sebelum koreksi pulsa
...
def convert_pulses_to_money(corrected_pulses):
    """Mengubah jumlah pulsa menjadi nominal uang setelah beberapa saat."""
    global total_inserted

    if corrected_pulses is None:
        return 0  # Tidak ada pulsa valid

    time.sleep(1)  # 🕒 Delay sebelum konversi uang
...
def deduct_from_balance(received_amount):
    """Mengurangi tagihan dari total uang yang masuk setelah beberapa detik."""
    global remaining_balance, transaction_active, waiting_pulses, id_trx

    time.sleep(1)  # 🕒 Delay sebelum pengurangan saldo
...
def count_pulse(gpio, level, tick):
    ...
    if (current_time - last_pulse_time) > DEBOUNCE_TIME:
        ...
        print(f"🔢 Pulsa diterima: {total_pulses}")  

    waiting_pulses = True

    # Proses pulsa setelah beberapa detik
    threading.Timer(PULSE_TIMEOUT, lambda: deduct_from_balance(convert_pulses_to_money(process_pulse_addition()))).start()
